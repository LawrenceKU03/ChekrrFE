...
const index: React.FC = () => {
	...
	const getProductData = async (chekrrId: string) => {
		...
		if(session_id){
			...
			const res_stripe_paid = await axios.post(
	...
	if (!stripeClientSecret || !productData) return <p className="text-white">Loading...</p>;
		...
		<div className=" bg-white h-full w-full py-6 flex md:flex-row flex-col items-start justify-center overflow-y-scroll">
			...
			>
				<ProductInfo
					...
					status={productData.is_paid ? "PAID" : "PENDING"}
				/>
				<MainCheckout
...
<FaCircleXmark  size={30}  className="ml-12 cursor-pointer" onClick={()=>stripeCheckout.setStripeCheckoutStatus(false)}/>
